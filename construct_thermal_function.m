function [theta, eta_vec, A, normalized_power_functions] = construct_thermal_function(alpha, k, thicknesses, pdens_cm2, power_functions, h, dimensions)
time_start = cputime;
...
% dimensions = 1 for 1D, 2, for 2D (cylindrical), 3 for 3D (spherical)
p = dimensions-1; % 0=cartesian, 1=cylinrical, 2=spherical
...
elseif dimensions == 2
    phis = besselj(0, a*x/sqrt(b));
    psis = bessely(0, a*x/sqrt(b));
    
    h1 = 0;

elseif dimensions == 3
    phis = cos(a*x/sqrt(b))/x;
    psis = sin(a*x/sqrt(b))/x;
...
if h1 ~=0
    ...
    null_list = zeros(2*num_layers-1, length(roots_vec));
    norm_list = zeros(2*num_layers-1, length(roots_vec));
end
...
if h1 ~= 0
    ...
    C_mat = [ norm_list(1,:) ; norm_list(2:2:end, :) ];
    D_mat = [ zeros(1,length(roots_vec)) ; norm_list(3:2:end, :)];
end
